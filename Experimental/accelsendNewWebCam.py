...
with open(path_pose, 'w') as file1, open(path_game, 'w') as file2, open(path_frames, 'w') as file3:
    ...
    while not keyboard.is_pressed("q"):#time.time() - init <=10
        ...
        #         countFrames = 0
        #     # frameTime = time.time()-init
        #     # cv2.putText(color_image, "{}".format(frameTime), (0, 25), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 0, 255), 2, cv2.LINE_AA)
        #     # cv2.putText(color_image, "{}".format(i), (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
        #     #             cv2.LINE_AA)
        #     # framepath = f'{dest_dir}/Frame_{i}_' + str(frameTime) + '.jpg'
        #     # cv2.imwrite(framepath, color_image)
        #     # # cv2.imshow('BGR feed', color_image)
        #     # # key = cv2.waitKey(1)
        #     # file3.write(
        #     #     str(i) + ',' + str(frameTime) + ',' + str(rateFrames) + '\n')
        #     # print(rateFrames)
        # except:
        #     flagFrames = 0
        ...
        if(flagA or flagB or flagC or flagD or flagE or flagG or flagN or flagFrames):
            writes +=1
            if writeCounter == 0:
            ...
            if flagFrames:
                cv2.putText(frame, "{}".format(timeWrite), (0, 25), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 0, 255), 2, cv2.LINE_AA)
                cv2.putText(frame, "{}".format(i), (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
                            cv2.LINE_AA)
