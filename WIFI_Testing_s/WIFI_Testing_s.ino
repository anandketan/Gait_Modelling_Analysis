...
char roll_1[255], pitch_1[255], yaw_1[255], accx[255], accy[255], accz[255], gyrx[255], gyry[255], gyrz[255], Mx[255], My[255], Mz[255];
...
#define BNO055_AXIS_MAP_CONFIG  0x24
#define BNO055_AXIS_MAP_SIGN    0x00
...
//    Serial.println("HIGH");
//  }
    if(millis()-cstart>1000){
//      Serial.println(millis()-start);
      crate = 1000*((float)ccount)/(millis()-cstart);
      cstart = millis();
      ccount = 0;
    }
    if(millis()-cstart<1000){
...
//    Serial.print(count);
    Serial.print("Calculation rate:");Serial.println( crate);
  timer =millis();
    readAccelData(accelCount);  // Read the x/y/z adc values
    // Now we'll calculate the accleration value into actual mg's
    ax = (float)accelCount[0] ;//+1;  // subtract off calculated accel bias
    ay = (float)accelCount[1] ;//+3;
    az = (float)accelCount[2];//+151;

    readGyroData(gyroCount);  // Read the x/y/z adc values
    // Calculate the gyro value into actual degrees per second
    gx = (float)gyroCount[0]/16.;//+0.19;  // subtract off calculated gyro bias
    gy = (float)gyroCount[1]/16.;//-0.25; // - gyroBias[1];  
    gz = (float)gyroCount[2]/16.;//-0.06; // - gyroBias[2];  

//    readMagData(magCount);  // Read the x/y/z adc values  
//    // Calculate the magnetometer values in milliGauss
//    mx = (float)magCount[0]/1.6 ;//-91.25;//  // get actual magnetometer value in mGauss
//    my = (float)magCount[1]/1.6;//+48.75;//
//    mz = (float)magCount[2]/1.6;//-435.62;  
...
//    Roll  = atan2(2.0f * (quat[0] * quat[1] + quat[2] * quat[3]),1.0f - 2.0f * (quat[1] * quat[1] + quat[2] * quat[2]));
...
//     Serial.print("Pressure_sensor = ");Serial.println(value);
//  
...
//    Serial.print(", ");
   ...
   Serial.print(", ");
   Serial.print("Strength = "); Serial.println(getStrength(10));
     ax = float(ax);
    ay = float(ay);
    az = float(az);
    gx = float(gx);
...
//    my = float(my);
//    mz = float(mz);
//    float roll_f = float(roll);
//    float pitch_f = float(pitch);
//    float yaw_f = float(yaw);
...
//    dtostrf(my, 4, 2, My);
//    dtostrf(mz, 4, 2, Mz);
//    dtostrf(roll_f, 4, 2, roll_1);
//    dtostrf(pitch_f, 4, 2, pitch_1);
//    dtostrf(yaw_f, 4, 2, yaw_1);
    ...
    udp.print(roll);//-3//15
    udp.print(',');
    udp.print(counter++);//16
    udp.print(','); //NTP requests are to port 123
    udp.print(timer);//17
    udp.print(',');
    udp.print(hs);//18
    udp.print(',');
    udp.print(distance);//19
    udp.endPacket();    
