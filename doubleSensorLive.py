...
ax.set_ylim([-100, 100])
line1, = ax.plot(x, y1, label='BaccX')
line2, = ax.plot(x, y2, label='BaccY')
line3, = ax.plot(x, y3, label='BaccZ')
line4, = ax.plot(x, y4, label='CaccX')
line5, = ax.plot(x, y5, label='CaccY')
line6, = ax.plot(x, y6, label='CaccZ')
...
while True:
    ...
    y4 = np.roll(y4, -1)
    y4[-1] = float(d2[gravaccx]) * 10
    y5 = np.roll(y5, -1)
    y5[-1] = float(d2[gravaccy]) * 10
    y6 = np.roll(y6, -1)
    y6[-1] = float(d2[gravaccz]) * 10
