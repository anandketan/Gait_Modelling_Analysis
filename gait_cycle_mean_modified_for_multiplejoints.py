...
for i in range(numberOfJoints):
    ...
    if column == 'diff_pitch_right':
        plt.plot(list(TimeAligned.keys()), RolledAvg, label='Right Knee')
    if column == 'diff_pitch_left':
        plt.plot(list(TimeAligned.keys()), ShiftedRolledAvg, label='Left Knee')
