...
def render_result(skeletons, color_img, depth_img, intr, confidence_threshold):
    ...
    if len(skeletons) == 1:
        for index, skeleton in enumerate(skeletons):
            ...
            if forward_angle < 165:
                if forward_angle < 145:
                    forwards = "F2"
                else:
                    forwards = "F1"
            elif forward_angle > 180:
                forwards = "Re"
            cv2.putText(color_img,"forward_angle={0:.4}".format(forward_angle),(850,25), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),2,cv2.LINE_AA)
            cv2.putText(color_img,"sway_angle={0:.4}".format(slope),(50,25), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),2,cv2.LINE_AA)
